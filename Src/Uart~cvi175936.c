	...
	if((RxData1.rxStopSign == 0x01) || (RxData2.rxStopSign == 0x01) || (Graph.pCurveArray->numofSmu1RxDots > Graph.pCurveArray->numOfTotalDots-1) || ((Graph.pCurveArray + 1)->numofSmu2RxDots > (Graph.pCurveArray + 1)->numOfTotalDots-1))
	{
		switch(TestPara1.testMode)
		{
			...
			case NO_SWEEP_RT:
				...
				if(select_Addr1 == 0x01)// 判断是哪一个板卡产生的停止，通过地址来判断。
				{
				
				
				}
				if(select_Addr1 == 0x02)// 2 号板子停止命令
				{
				
				}
				if((select_Addr1 == 0x01) && (select_Addr1 == 0x02))
				{
...
void Getxy(unsigned char *measUartRxBuf, RxDataTypeDef* RxData1, RxDataTypeDef* RxData2) 							//判断接收源表1 源表2的数据
{
		...
		else if((*measUartRxBuf  == 0x02) && ((Graph.pCurveArray + 1)->numofSmu2RxDots <= (Graph.pCurveArray + 1)->numOfTotalDots-1)) //判断是否是 02 地址传来 的数据
		{
			...
			{
				*((Graph.pCurveArray +1)->pDotX++) = (Graph.X_Axis1++)*TestPara1.timeStep*0.001;
				*((Graph.pCurveArray +1)->pDotY++) = RxData1->rx_current.num_float;										//get y, set pointer to the next data 
		...
		while(rxNum>=15)
		{
			if(Graph.pCurveArray->numOfPlotDots >0)
			{
				...
				if(Rx_CGS_Data.rxDevAddr == 0x11)
				{
						...
						for(i = 0; i<j; i++)				  
						{
							if((TestPara1.testMode == NO_SWEEP_RT) || (TestPara1.testMode == NO_SWEEP_IT))
							{
							...
							if((TestPara1.testMode == NO_SWEEP_IV) || (TestPara1.testMode == NO_SWEEP_VI))
							{
							...
							*((Graph_Temp.pCurveArray + 1)->pDotY++) = Rx_CGS_Data.heating_stage_temp;		 //热台温度     
							*((Graph_Temp.pCurveArray + 2)->pDotY++) = Rx_CGS_Data.pressure * 0.001;		 //压强
							...
							SetTableCellVal (tablePanel, TABLE_TABLE1, MakePoint (7,rowIndex3), *((Graph_Temp.pCurveArray + 2)->pDotY-1));
							rowIndex3++;
